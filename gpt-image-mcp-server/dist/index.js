...
function assertAllowedPath(p) {
    ...
    if (!allowedRoots().some((root) => isWithin(resolved, root))) {
        throw new ImageAPIError(`Path ${p} is outside the allowed roots (home + IMAGE_ALLOWED_ROOTS). ` +
...
function asciiStem(name, index) {
    const ext = path.extname(name).toLowerCase();
    const stem = path.basename(name, ext).replace(/[^A-Za-z0-9_-]+/g, "-").replace(/^-+|-+$/g, "") ||
        `image${index + 1}`;
    return stem.slice(0, 40);
}
function asciiSafeName(name, index) {
    const ext = path.extname(name).toLowerCase();
 ...
 */
async function resolveImageInput(ref, index) {
    const s = ref.trim();
    if (!s)
        throw new ImageAPIError("Image reference must not be empty.");
    ...
    if (dataUri) {
        ...
        if (!buffer.length)
            throw new ImageAPIError("data: URI contains no image data.");
    ...
    if (/^https?:\/\//i.test(s)) {
        ...
        try {
            ...
            throw new ImageAPIError(`Could not download image ${s}: ${err instanceof Error ? err.message : String(err)}`);
        }
        if (!buffer.length)
            throw new ImageAPIError(`Image URL returned no data: ${s}`);
    ...
    if (fs.existsSync(s) && fs.statSync(s).isFile()) {
        ...
        if (!buffer.length)
            throw new ImageAPIError(`Image file is empty: ${s}`);
        return { buffer, filename: asciiSafeName(path.basename(s), index), mime: mimeForName(s) };
    }
    if (s.length >= 64 && RAW_BASE64_RE.test(s)) {
        const buffer = Buffer.from(s, "base64");
        if (buffer.length)
            return { buffer, filename: `image${index + 1}.png`, mime: "image/png" };
    }
    throw new ImageAPIError(`Unrecognized image input: "${s.slice(0, 60)}${s.length > 60 ? "..." : ""}". ` +
        "Provide a local file path, an http(s) URL, a data: URI, or raw base64.");
}
function saveImageFile(data, prompt, saveDir) {
...
async function formatImagesResult(result, ctx) {
    const typed = result;
...
async function previewBlocks(buffers, count) {
    const blocks = [];
...
function redactError(err, cfg) {
    let msg = err instanceof Error ? err.message : String(err);
    if (cfg.api_key)
        msg = msg.split(cfg.api_key).join("<redacted>");
 ...
 */
async function editImage(images, prompt, opts) {
    if (!prompt || !prompt.trim())
        throw new ImageAPIError("`prompt` must not be empty.");
    ...
    if (refs.length > 10)
        throw new ImageAPIError("`images` supports at most 10 images.");
    const cfg = loadConfig();
    if (!cfg.api_key) {
        throw new ImageAPIError("API key is not set. Set the IMAGE_API_KEY environment variable, " +
            "or use set_config(api_key=...).");
    ...
    if (cfg.moderation.enabled)
        await moderate(prompt, cfg.moderation);
    const files = await Promise.all(refs.map((ref, i) => resolveImageInput(ref, i).then((img) => normalizeImage(img, `input image ${i + 1}`))));
    ...
    if (opts.mask) {
        ...
        if (!maskRef)
            throw new ImageAPIError("`mask` must not be empty.");
    ...
    const result = await httpJson(`${cfg.api_base_url.replace(/\/+$/, "")}/images/edits`, cfg.api_key, { method: "POST", body: form });
    return formatImagesResult(result, {
        prefix: `Edited ${files.length} input image(s)` +
            (opts.mask ? " + mask" : "") +
...
server.registerTool("generate_image", {
    ...
    try {
        ...
        if (includePreview)
            content.push(...(await previewBlocks(result.buffers, previewCount)));
        return {
            content,
            structuredContent: {
                ok: true,
        ...
        return {
            content: [{ type: "text", text: `Error: ${redactError(err, loadConfig())}` }],
            isError: true,
        };
    }
});
server.registerTool("edit_image", {
    ...
    description: "Edit, transform, combine or extend one or more input images following a text " +
    ...
    inputSchema: z.object({
        images: z
            ...
            .describe("1-10 input image references: local file path, http(s) URL, data: URI or " +
            "raw base64. A single string is also accepted."),
        prompt: z.string().describe("What to change or how to transform the image(s)."),
        mask: z.string().optional().describe("Optional mask image (local path / URL / data URI / base64) marking the " +
